        ...
        if (gesture_type == wp->config_attr[0].val
            ...
            do {
                if (0 ==
                    ...
                    GeisFloat flv = attrs[attrsI].float_val;
                    GeisFloat val = wp->config_attr[cAttrI].val;
                if(abs(val)>abs(maxVal)){
                    maxVal=flv;
                }
                    GeisFloat valMax = wp->config_attr[cAttrI].valMax;
                    printf("DEBUG -- comparing %s and %s : ",
                           attrs[attrsI].name,
                           wp->config_attr[cAttrI].attrName);
                    printf("%.2f %.2f %.2f is inside: ",
                    ...
                    printf("%i \n",
                           inside(attrs[attrsI].float_val,
                                  wp->config_attr[cAttrI].val,
                                  wp->config_attr[cAttrI].valMax));
                    //if this is a finish motion, we only care about the maxVal
                    if(wp->when==GINN_FINISH){
                        printf("This is a finish gesture: %d\n", wp->when);
                        if(inside(maxVal, val, valMax)){
                           //then we say that the motion has taken palce
                           valid=1;
                        }else{
